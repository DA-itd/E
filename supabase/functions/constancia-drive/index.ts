...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rutaArchivo = `${tipo}/${body.nombreArchivo || `${docenteId}_${cursoId}.pdf`}`
    ...
    if (accion === 'obtener') {
      ...
      const { data: archivo, error: errorDescarga } = await supabaseAdmin
        .storage.from(BUCKET).download(existente.storage_path)
    ...
    if (accion === 'guardar') {
      ...
      await supabaseAdmin.from('constancias_generadas').upsert(
        { tipo, docente_id: docenteId, curso_id: cursoId, storage_path: rutaArchivo },
