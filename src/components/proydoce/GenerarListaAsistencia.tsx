...
import { supabase, getLocalCursos, saveLocalCursos, getLocalDocentes, saveLocalDocentes } from '../../lib/supabaseClient';
...
export default function GenerarListaAsistencia({ cursoId, cursoProp, onClose }: Props) {
  ...
  async function cargarCatalogoDocentes() {
    ...
    // 1. Docentes del catálogo oficial Supabase (revisando tablas posibles)
    try {
      const tablas = ['docentes', 'usuarios', 'personal', 'profesores', 'personal_docente'];
      for (const tabla of tablas) {
        try {
          const { data: docs } = await supabase.from(tabla).select('*');
          if (Array.isArray(docs) && docs.length > 0) {
            docs.forEach((d: any) => {
              const nombreRaw = (
                ...
                d.nombre || 
                d.nombres || 
                (d.apellido_paterno ? `${d.apellido_paterno} ${d.apellido_materno || ''} ${d.nombre || d.nombres || ''}` : '') ||
                (d.apellidos ? `${d.nombre || d.nombres || ''} ${d.apellidos}` : '') ||
                ''
              ).trim();
              if (!nombreRaw) return;
              const norm = normalizar(nombreRaw);
              const curpVal = (d.curp || d.CURP || d.clave_curp || '').trim().toUpperCase();
              const rfcVal = (d.rfc || d.RFC || d.clave_rfc || '').trim().toUpperCase();
              const rfcCurp = calcularRfcCurp(nombreRaw, rfcVal, curpVal);
              const deptoRaw = d.departamento || d.depto || d.adscripcion || d.area || d.puesto || '';
              const nivelEst = d.nivel_estudios || d.grado_academico || d.grado || d.escolaridad || (
                nombreRaw.startsWith('DR') ? 'Doctorado' : 
                (nombreRaw.startsWith('M.C') || nombreRaw.startsWith('MTRO') || nombreRaw.startsWith('MTR') || nombreRaw.startsWith('MAE')) ? 'Maestría' : 
                nombreRaw.startsWith('ING') ? 'Ingeniería' : 'Licenciatura'
              );

              if (!mapaDocentes.has(norm)) {
                mapaDocentes.set(norm, {
                  id: d.id || `doc-${norm}`,
                  nombre_completo: nombreRaw.toUpperCase(),
                  rfc: d.rfc || rfcCurp.rfc,
                  curp: d.curp || rfcCurp.curp,
                  email: (d.email || d.correo || d.correo_institucional || '').trim().toLowerCase(),
                  telefono: (d.telefono || d.tel || d.celular || d.contacto || '').trim(),
                  departamento: limpiarPrefijosDocente(deptoRaw),
                  puesto: d.puesto || d.categoria || 'Docente',
                  puesto_departamento: `${d.puesto || 'Docente'} - ${limpiarPrefijosDocente(deptoRaw)}`,
                  nivel: d.nivel || (d.es_fd ? 'Funcionario Docente' : 'Docente'),
                  nivel_estudios: nivelEst,
                  es_fd: Boolean(d.es_fd || d.rol === 'admin' || d.nivel === 'Funcionario Docente' || (d.puesto && d.puesto.toLowerCase().includes('funcionario'))),
                  es_d: Boolean(d.es_d !== undefined ? d.es_d : !d.es_fd),
                  genero: d.genero || d.sexo || (esMujer(nombreRaw) ? 'Femenino' : 'Masculino'),
                  tarjeta: (d.tarjeta || d.no_tarjeta || d.num_tarjeta || d.numero_tarjeta || d.clave || d.no_empleado || '').trim(),
                  rol: d.rol || 'docente'
                });
              } else {
                const ex = mapaDocentes.get(norm);
                if (!ex.curp && (d.curp || rfcCurp.curp)) ex.curp = d.curp || rfcCurp.curp;
                if (!ex.rfc && (d.rfc || rfcCurp.rfc)) ex.rfc = d.rfc || rfcCurp.rfc;
                if (!ex.email && (d.email || d.correo)) ex.email = (d.email || d.correo).trim().toLowerCase();
                if (!ex.telefono && (d.telefono || d.tel || d.celular)) ex.telefono = (d.telefono || d.tel || d.celular).trim();
                if (!ex.departamento && deptoRaw) ex.departamento = limpiarPrefijosDocente(deptoRaw);
                if (!ex.nivel_estudios && nivelEst) ex.nivel_estudios = nivelEst;
                if (!ex.tarjeta && (d.tarjeta || d.no_tarjeta)) ex.tarjeta = (d.tarjeta || d.no_tarjeta).trim();
              }
            });
          }
        } catch {
          // Ignorar tabla si no existe en Supabase
        }
      }
    } catch (e) {
      console.warn('Error cargando docentes supabase:', e);
    }

    // 2. Docentes del catálogo local (localStorage)
    try {
      const localDocs = getLocalDocentes();
      localDocs.forEach((d: any) => {
        const nombre = (d.nombre_completo || d.nombre || '').trim();
        if (!nombre) return;
        const norm = normalizar(nombre);
        const curpVal = (d.curp || d.CURP || '').trim().toUpperCase();
        const rfcVal = (d.rfc || d.RFC || '').trim().toUpperCase();
        const rfcCurp = calcularRfcCurp(nombre, rfcVal, curpVal);
        ...
        if (!mapaDocentes.has(norm)) {
          mapaDocentes.set(norm, {
            ...
            rfc: d.rfc || rfcCurp.rfc,
            curp: d.curp || rfcCurp.curp,
            email: (d.email || d.correo || '').trim().toLowerCase(),
            telefono: (d.telefono || d.tel || d.celular || '').trim(),
            departamento: limpiarPrefijosDocente(d.departamento || ''),
            puesto: d.puesto || 'Docente',
            puesto_departamento: d.puesto_departamento || `${d.puesto || 'Docente'} - ${d.departamento || ''}`,
            nivel: d.nivel || (d.es_fd ? 'Funcionario Docente' : 'Docente'),
            nivel_estudios: d.nivel_estudios || d.grado_academico || (nombre.startsWith('DR') ? 'Doctorado' : (nombre.startsWith('M.C') || nombre.startsWith('MTRO')) ? 'Maestría' : nombre.startsWith('ING') ? 'Ingeniería' : 'Licenciatura'),
            es_fd: Boolean(d.es_fd || d.rol === 'admin' || d.nivel === 'Funcionario Docente'),
            es_d: Boolean(d.es_d !== undefined ? d.es_d : !d.es_fd),
            genero: d.genero || (esMujer(nombre) ? 'Femenino' : 'Masculino'),
            tarjeta: (d.tarjeta || d.no_tarjeta || '').trim(),
            rol: d.rol || 'docente'
          });
        } else {
          const existente = mapaDocentes.get(norm);
          if (!existente.curp && (d.curp || rfcCurp.curp)) existente.curp = d.curp || rfcCurp.curp;
          if (!existente.rfc && (d.rfc || rfcCurp.rfc)) existente.rfc = d.rfc || rfcCurp.rfc;
          if (!existente.email && d.email) existente.email = d.email;
          if (!existente.telefono && d.telefono) existente.telefono = d.telefono;
          if (!existente.tarjeta && d.tarjeta) existente.tarjeta = d.tarjeta;
          if (!existente.departamento && d.departamento) existente.departamento = d.departamento;
          if (!existente.nivel_estudios && d.nivel_estudios) existente.nivel_estudios = d.nivel_estudios;
          if (!existente.genero && d.genero) existente.genero = d.genero;
        }
      });
    } catch (e) {
      console.warn('Error cargando docentes locales:', e);
    }

    // 3. Participantes de todos los cursos guardados (histórico de docentes inscritos)
    try {
      ...
      cursos.forEach((c) => {
        if (Array.isArray(c.participantes)) {
          c.participantes.forEach((p: any) => {
            ...
            if (!nombre) return;
            const norm = normalizar(nombre);
            const curpVal = (p.curp || p.CURP || '').trim().toUpperCase();
            const rfcVal = (p.rfc || p.RFC || '').trim().toUpperCase();
            const rfcCurp = calcularRfcCurp(nombre, rfcVal, curpVal);
            const depto = limpiarPrefijosDocente(p.departamento || p.puesto_departamento || p.puesto || c.departamento || '');

            if (!mapaDocentes.has(norm)) {
              mapaDocentes.set(norm, {
                id: p.id || `doc-${norm}`,
                nombre_completo: nombre.toUpperCase(),
                rfc: p.rfc || rfcCurp.rfc,
                curp: p.curp || rfcCurp.curp,
                email: (p.email || p.correo || '').trim().toLowerCase(),
                telefono: (p.telefono || p.tel || '').trim(),
                departamento: depto,
                puesto: p.puesto || 'Docente',
                puesto_departamento: p.puesto_departamento || `${p.puesto || 'Docente'} - ${depto}`,
                nivel: p.nivel || (p.es_fd ? 'Funcionario Docente' : 'Docente'),
                nivel_estudios: p.nivel_estudios || (nombre.startsWith('DR') ? 'Doctorado' : (nombre.startsWith('M.C') || nombre.startsWith('MTRO')) ? 'Maestría' : nombre.startsWith('ING') ? 'Ingeniería' : 'Licenciatura'),
                es_fd: Boolean(p.es_fd || p.nivel === 'Funcionario Docente'),
                es_d: Boolean(p.es_d !== undefined ? p.es_d : !p.es_fd),
                genero: p.genero || (esMujer(nombre) ? 'Femenino' : 'Masculino'),
                tarjeta: (p.tarjeta || p.no_tarjeta || '').trim(),
                rol: 'docente'
              });
            } else {
              const existente = mapaDocentes.get(norm);
              if (!existente.curp && (p.curp || rfcCurp.curp)) existente.curp = p.curp || rfcCurp.curp;
              if (!existente.rfc && (p.rfc || rfcCurp.rfc)) existente.rfc = p.rfc || rfcCurp.rfc;
              if (!existente.departamento && depto) existente.departamento = depto;
              if (!existente.email && p.email) existente.email = p.email;
              if (!existente.telefono && p.telefono) existente.telefono = p.telefono;
              if (!existente.tarjeta && p.tarjeta) existente.tarjeta = p.tarjeta;
              if (!existente.nivel_estudios && p.nivel_estudios) existente.nivel_estudios = p.nivel_estudios;
            }
          });
        }
      });
    } catch (e) {
      console.warn('Error leyendo histórico de docentes:', e);
    }

    const lista = Array.from(mapaDocentes.values()).sort((a, b) =>
      a.nombre_completo.localeCompare(b.nombre_completo)
    );
    setCatalogoDocentes(lista);
  }

  // Filtrado reactivo ultra-preciso por subcadenas y palabras múltiples
  const sugerenciasDocentes = useMemo(() => {
    ...
    return catalogoDocentes
      .filter((d) => {
        ...
        const tarj = (d.tarjeta || '').toUpperCase();

        // 1. Coincidencia si todas las palabras buscadas aparecen en el nombre, depto o puesto
        const coincidePalabras = palabrasQuery.length > 0 && palabrasQuery.every((pal) =>
          nNorm.includes(pal) || deptoNorm.includes(pal) || pNorm.includes(pal) || r.includes(pal.toUpperCase())
        );

        // 2. Coincidencia directa en cualquiera de los campos
        const coincideDirecto =
          ...
          em.includes(qUpper) ||
          tarj.includes(qUpper) ||
          (rawQuery.length >= 4 && tel.includes(rawQuery.replace(/\D/g, '')));

        return coincidePalabras || coincideDirecto;
      })
      .slice(0, 15);
  }, [catalogoDocentes, nuevoNombre]);
  ...
  function handleSeleccionarDocente(doc: any) {
    setNuevoNombre(doc.nombre_completo);
    setNuevoRfc(doc.rfc || '');
    setNuevoCurp(doc.curp || '');
    setNuevoEmail(doc.email || `${normalizar(doc.nombre_completo).split(' ')[0].toLowerCase()}@itdurango.edu.mx`);
    setNuevoTelefono(doc.telefono || '');
    setNuevoDepartamento(doc.departamento || datosCurso?.departamento || '');
    setNuevoPuesto(doc.puesto || (doc.es_fd ? 'Funcionario Docente' : 'Docente'));
    setNuevoNivelEstudios(doc.nivel_estudios || 'Licenciatura');
    setNuevoTipo(doc.es_fd ? 'FD' : 'D');
    const gen = doc.genero ? (doc.genero === 'H' || doc.genero === 'Hombre' || doc.genero === 'Masculino' ? 'Masculino' : 'Femenino') : (esMujer(doc.nombre_completo) ? 'Femenino' : 'Masculino');
    setNuevoGenero(gen);
    setNuevaTarjeta(doc.tarjeta || '');
  ...
  function handleGuardarNuevoParticipante(e: React.FormEvent) {
    ...
    try {
      ...
      supabase.from('docentes').upsert(docenteRegistro).then(() => {});
    } catch (err) {
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <div>
                <h3 className="font-bold text-base text-[#1B396A] flex items-center gap-2">
                  <span>➕</span> Registro de Participante / Docente
                </h3>
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Autocompleta desde el catálogo o ingresa todos los datos del registro docente
                </p>
              </div>
              <button
              ...
              <div className="relative">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {catalogoDocentes.length > 0 && (
                    <span className="text-[10px] text-blue-600 font-medium flex items-center gap-1 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-200">
                      <span className="inline-block w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></span>
                      {catalogoDocentes.length} docentes en base de datos
                    </span>
                  )}
                </div>
                ...
                <div className="relative">
                  <input
                    type="text"
                    required
                    placeholder="Escriba las primeras letras del nombre (Ej. SOTO, LAURA, AGUIRRE...)"
                    value={nuevoNombre}
                    onChange={(e) => {
                    ...
                    onFocus={() => {
                      if (nuevoNombre.trim().length >= 1) setMostrarSugerencias(true);
                    }}
                    onKeyDown={(e) => {
                ...
                {mostrarSugerencias && sugerenciasDocentes.length > 0 && (
                  <div className="absolute z-50 left-0 right-0 mt-1 bg-white border border-blue-300 rounded-xl shadow-2xl max-h-64 overflow-y-auto divide-y divide-slate-100 animate-in fade-in zoom-in-95 duration-100">
                    <div className="px-3 py-1.5 bg-blue-50/90 text-[10px] font-bold text-blue-900 uppercase tracking-wider flex justify-between items-center sticky top-0 z-10 border-b border-blue-200">
                      <span>Coincidencias en Base de Datos ({sugerenciasDocentes.length})</span>
                      <span className="text-[9px] font-normal text-blue-600">Clic para auto-llenar todos los campos</span>
                    </div>
                    {sugerenciasDocentes.map((doc, idx) => (
                      <button
                        ...
                        className={`w-full text-left px-3.5 py-2.5 transition flex flex-col gap-1 cursor-pointer border-b border-slate-100 last:border-0 ${
                          docenteSeleccionadoIndex === idx
                            ? 'bg-blue-100/90 text-blue-950 border-l-4 border-[#1B396A]'
                            : 'hover:bg-slate-50 text-slate-800'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-xs text-[#1B396A]">
                            {doc.nombre_completo}
                          </span>
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-500">
                          {doc.departamento && (
                            <span className="font-medium text-slate-700">🏢 {doc.departamento}</span>
                          )}
                          {doc.genero && (
                            <span className="text-[10px] text-slate-600 bg-slate-100 px-1 rounded">👤 {doc.genero}</span>
                          )}
                          {doc.curp && (
                            <span className="font-mono text-[10px] text-slate-700 bg-slate-100 px-1 rounded">CURP: {doc.curp}</span>
                          )}
                          {doc.rfc && (
                ...
                {docenteAutocompletado && (
                  <p className="text-[11px] text-emerald-700 font-medium mt-1.5 flex items-center gap-1.5 bg-emerald-50 border border-emerald-200 p-2 rounded-lg">
                    <span>✨</span>
                    <span><strong>Docente cargado con éxito:</strong> CURP, RFC, Correo, Teléfono, Departamento, Género y Nivel de Estudios autocompletados desde la base de datos de Docentes.</span>
                  </p>
                )}
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      value={nuevoCurp}
                      onChange={(e) => setNuevoCurp(e.target.value.toUpperCase())}
              ...
              <div className="bg-slate-50/80 p-3 rounded-xl border border-slate-200 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                    ...
                    <div className="flex gap-3 pt-1.5">
                      ...
                      <label className="flex items-center gap-1.5 cursor-pointer">
                        <input
                          ...
                          className="text-blue-600"
                        />
                        <span className="font-semibold text-slate-800 text-[11px]">FD (Funcionario)</span>
                      </label>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="Ej. T-1045"
                      value={nuevaTarjeta}
                      onChange={(e) => setNuevaTarjeta(e.target.value.toUpperCase())}
                      className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs bg-white uppercase focus:ring-2 focus:ring-blue-500 outline-none"
                    />
                  </div>
                </div>
              </div>
